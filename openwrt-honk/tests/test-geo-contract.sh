...
service="$repo_root/luci-app-honk/luasrc/model/service.lua"
...
grep -F 'local GEO_DIR = os.getenv("HONK_GEO_DIR") or "/usr/share/v2ray"' "$service" >/dev/null || fail "LuCI Geo path is not OpenWrt-owned"
...
if rg -n 'GEO_(SITE|IP)_CACHE|geo\.lock|LOYALSOLDIER_LOCKED|V2FLY_LOCKED|DAE_ALLOW_CUSTOM_GEO|geo capabilities|geo repair|/usr/(lib|share)/honk' "$makefile" "$init" "$repo_root/honk/files/quick-transaction-worker" "$service" "$repo_root/luci-app-honk/luasrc/controller/honk.lua" "$repo_root/luci-app-honk/ui/src" "$repo_root/luci-app-honk-legacy/luasrc" "$repo_root/luci-app-honk-legacy/ui/src" "$repo_root/.github/workflows" >/dev/null; then
	fail "Honk-owned or locked Geo implementation remains"
fi
if rg -n 'geo_settings|geo_download|geosite_url|geoip_url|allow_custom_geo' "$repo_root/honk/files" "$repo_root/luci-app-honk" "$repo_root/luci-app-honk-legacy" "$repo_root/.github/workflows" --glob '!**/root/www/**' >/dev/null; then

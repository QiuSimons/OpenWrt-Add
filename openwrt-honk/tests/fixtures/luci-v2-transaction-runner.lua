...
package.preload["luci.jsonc"] = function()
	return {
		parse = function(value) if tostring(value):find('"ok":true', 1, true) then return { ok = true } end return nil end,
...
package.preload["luci.sys"] = function()
	return {
		...
		exec = function() return '{"ok":true}' end,
	}
end

for _, name in ipairs({ "config", "dns", "node", "mode", "honk_network", "service" }) do
...
if arg[4] == "success" then
	...
	elseif arg[4] == "interfaces" then
		local result, status = service.apply_interfaces({ lanDevice = "br-lan", wanDevice = "eth0.2", dialMode = "domain++", expectedRevision = expected })
